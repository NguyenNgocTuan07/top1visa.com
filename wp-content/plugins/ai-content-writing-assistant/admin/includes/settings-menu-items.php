...
function aiwa_add_excerpt_settings_items(){
    ...
    <div class="settings-item">
        <label for="aiwa-add-excerpt"><span><?php _e('Add excerpt', 'ai-writing-assistant'); ?></span>
            <input id="aiwa-add-excerpt" class="content-settings-input" type="checkbox" name="add-excerpt" data-has-subsettings="" <?php echo esc_attr(get_option($key.'add-excerpt', 'on')) =='on' ? 'checked': ''; ?>>
        </label>
        <p><?php _e('Add conclusion end of the topics.', 'ai-writing-assistant'); ?></p>
    </div>

    <div class="settings-item sub-settings-item  <?php echo esc_attr(get_option($key.'add-excerpt', 'on')) =='on' ? '': 'aiwa-hidden'; ?>" data-subsettings-of="aiwa-add-excerpt">
