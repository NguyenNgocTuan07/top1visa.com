...
// $data_comment = json_encode($results);
// @$wc_get_order_notes = json_decode($data_comment,true);
...
$data_comment = json_encode($results);
@$wc_get_order_notes = array_reverse( json_decode($data_comment), true );
// print_r($wc_get_order_notes);
...
<div class="box_tracking_by_neko section-content relative">
	...
	<div class="row">
		...
		?>
			<div class="timeline-item <?php echo ($order_report_item['date'] != '') ? 'active' : '';?>">
			<div class="timeline-content">
			  <h2><?php echo ($order_report_item['date'] == '') ? '<i class="fa fa-clock-o" aria-hidden="true"></i>' : '<i class="fa fa-check" aria-hidden="true"></i>';?> <?php echo $order_report_item['title'];?></h2>
			  <p><?php echo $order_report_item['content'];?></p>
			  <?php echo ($order_report_item['date'] != '') ? '<span class="timeline-date"><i class="fa fa-clock-o" aria-hidden="true"></i> '.$order_report_item['date'] : '';?></span>
			</div>
			</div>
		<?php } ?>
		</div>
		...
		<div class="chat-container">
		  ...
		  <div class="chat-messages">
			<?php  foreach($wc_get_order_notes as $customer_note_item) {
				$date = date_create($customer_note_item->date_created->date);
				$data_new = date_format($date,"d/m/Y H:i:s");
		
			?>
			<div class="message <?php echo ($customer_note_item->customer_note == 1) ? 'dsadsa' : 'vxz';?>">
			  <div class="message-sender">
				<?php echo ($customer_note_item->customer_note == '1') ? '<i class="fa fa-user-secret" aria-hidden="true"></i> Khách Hàng' : '<i class="fa fa-user" aria-hidden="true"></i> Quản Trị';?>
			  </div>
			...
			<form id="Send_Comment" action="" method="POST">
				...
				<div class="send_chat_form">
					...
					<div class="upload">
					  ...
					  <input multiple="multiple" id="file-upload" type="file" class="file-upload-input" name="file[]" />
					</div>
				</div>	
...
jQuery(document).ready(function(){
  jQuery('input[type="file"]').change(function(e){
    // var fileCount = e.target.files[0].files.length;
    // jQuery('.file-name').text(fileName);
	
	var fileCount = jQuery(this)[0].files.length;
    jQuery('.file-name').text(fileCount + " Files");
...
jQuery(document).ready(function () {
	...
	setInterval(function(){load_ajax_chat()}, 3000);
	function load_ajax_chat() {
		jQuery.get("<?php echo get_home_url();?>/wp-admin/admin-ajax.php?action=load_chat_ajax&order_id=<?php echo $order_id;?>", function(data, status){
...
function vithanhlam_send_chat_ajax() {
	...
	if(!empty($_FILES['file']['name'])) {
		...
		foreach ($uploaded_file['name'] as $key => $value) {
			$file = array(
			'name' => $value,
			'type' => $uploaded_file['type'][$key],
			'tmp_name' => $uploaded_file['tmp_name'][$key],
			'error' => $uploaded_file['error'][$key],
			'size' => $uploaded_file['size'][$key]
			);
			$upload = wp_handle_upload($file, $upload_overrides);
	
			$filess.=' 
<a class="file_download" target="_blank" href="'.$upload['url'].'"> <i class="fa fa-download" aria-hidden="true"></i> '.$value.'</a></br>';
	...
	$telegram = 'OrderID '.$order_id.': '.$text. strip_tags($filess);
...
?>
			...
			<?php  foreach($wc_get_order_notes as $customer_note_item) {
			$dates = date_create($customer_note_item->date_created->date);
			$data_new = date_format($dates,"d/m/Y H:i:s");
				?>
			<div class="message <?php echo ($customer_note_item->customer_note == 1) ? 'dsadsa' : 'vxz';?>">
			  <div class="message-sender">
				<?php echo ($customer_note_item->customer_note == '1') ? '<i class="fa fa-user-secret" aria-hidden="true"></i> Khách Hàng' : '<i class="fa fa-user" aria-hidden="true"></i> Quản Trị';?>
