...
$wpaicg_trackings_sql = "SELECT p.*,(".$wpaicg_sum_length.") as word_count,(".$wpaicg_sum_time.") as time_run,(".$wpaicg_sum_tokens.") as total_tokens FROM ".$wpdb->posts." p WHERE p.post_type='wpaicg_tracking' AND p.post_status IN ('publish','pending','draft','trash') ORDER BY p.post_date DESC LIMIT ".$wpaicg_tracking_offset.",".$wpaicg_tracking_per_page;
$wpaicg_trackings_total_sql = "SELECT COUNT(*) FROM ".$wpdb->posts." p WHERE p.post_type='wpaicg_tracking' AND p.post_status IN ('publish','pending','draft','trash')";
    ...
    if($wpaicg_trackings && is_array($wpaicg_trackings) && count($wpaicg_trackings)):
        foreach($wpaicg_trackings as $wpaicg_tracking):
            ...
            <tr>
                ...
                <td><a onclick="return confirm('Are you sure?')" class="button button-link-delete button-small" href="<?php echo admin_url('admin.php?page=wpaicg_bulk_content&wpaicg_action=tracking&sub_action=delete&id='.$wpaicg_tracking->ID)?>">Delete</a></td>
            </tr>
        <?php
        endforeach;
    endif;
